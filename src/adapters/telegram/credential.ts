...
export async function encryptTelegramCredential(
  env: Env,
  plaintext: string
): Promise<EncryptedTelegramCredential> {
  const masterKey = resolveCredentialMasterKey(env);
  if (!masterKey) {
    throw new Error("AGENT_MASTER_KEY or INTERNAL_ADMIN_TOKEN is required to save Telegram tokens");
  }

  return encryptString(plaintext, masterKey);
}

export async function resolveTelegramCredential(
  env: Env,
  ...
  const masterKey = resolveCredentialMasterKey(env);
  if (!masterKey) {
    throw new Error("AGENT_MASTER_KEY or INTERNAL_ADMIN_TOKEN is required to read Telegram tokens");
  }
