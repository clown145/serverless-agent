...
import { trySendQqOfficialAttachment } from "../../adapters/qq/official/attachment-sender";
...
export class QQOfficialGatewayDurableObject {
  ...
  private async send(
    ...
    if (target.kind === "group") {
      ...
      return sendAttachmentResult(result);
    }
    if (target.kind === "c2c") {
      if (!input.file) {
