...
export const createWeixinOcIntegrationSchema = z.object({
  ...
  qrPollIntervalMs: z.number().int().min(1_000).max(60_000).default(DEFAULT_WEIXIN_OC_QR_POLL_INTERVAL_MS),
  longPollTimeoutMs: z.number().int().min(5_000).max(110_000).default(DEFAULT_WEIXIN_OC_LONG_POLL_TIMEOUT_MS),
